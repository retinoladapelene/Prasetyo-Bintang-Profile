...
import { GALLERY_CONFIG, SPIDERMAN_CONFIG, GALLERY_TRAVEL } from "./CylinderGalleryConfig";
import { generateEnvironmentMap } from "./GalleryUtils";

export interface CylinderGallerySceneProps {
  items: Array<any>;
  wallItems?: Array<any>;
...
export function useCylinderGallery({ items, wallItems, smoothProgress, className, onActiveIndexChange, onAvatarReady, activeThemeId, onThemeChange }: CylinderGallerySceneProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Rotate the entire group based on smoothed scroll (synced with ArchiveGallery exactIdx logic)
      const carouselProgress = Math.min(smoothedScroll / 0.85, 1.0);
      const exactIdx = (carouselProgress * totalRotationSteps) - paddingStart;
      carouselGroup.rotation.y = -exactIdx * anglePerItem;
      ...
      carouselGroup.position.y = (carouselProgress * totalTranslation) - (paddingStart * verticalStagger) + floatOffset + 0.35;
